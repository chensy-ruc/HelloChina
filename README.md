...
- 中国主题视觉与展示入口
  ...
  - 首页开头的三个主题标签会随当前城市与界面语言自动切换，不再固定显示北京相关内容
  ...
  - 公开 SVG 素材已本地化保留为设计参考，素材来源与许可记录在 `assets/traditional/README.md`
  - 当前展示元素已进一步统一为 CSS 绘制的折扇、如意云、中国结、玉璧与青花水波，视觉风格与灯笼一致
  - 传统元素已从独立图标堆叠改为低层级背景纹理，避免在板块中占位或喧宾夺主
...
- 山水云纹与梅兰竹菊曾尝试升级为公开来源的本地 SVG 素材，当前这些素材已保留为设计参考
- 新增 `assets/traditional/README.md`，记录各 SVG 素材的来源、许可与页面使用位置
- 页面已移除直接展示山水云纹与梅兰竹菊 SVG 的方案，改为更适合页面嵌入的 CSS 折扇、如意云、中国结、玉璧与青花水波元素
- 二维码入口区的传统元素已从 CSS 图形按钮调整为背景化云纹与折扇纹理，避免素材风格不一致和元素堆叠感
- 传统元素已进一步改为背景化嵌入：Hero 区作为低透明度视觉纹理，二维码入口区则通过背景云纹与折扇形态自然融入版面
- 首页开头的三个黄色主题标签已改为城市联动：北京、上海、西安、成都、杭州各自展示对应城市气质，并支持 `EN / 中文 / JA / KO / ES / FR / AR`

## 下一步的改进任务
- 当前 README 中列出的改进任务已完成，暂无新的待处理项
